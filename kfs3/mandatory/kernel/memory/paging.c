/*
 * Paging Implementation
 * Manages page tables and page directories for virtual memory
 */

#include "memory.h"
#include "screen.h"
#include "panic.h"

/* Page directory (1024 entries, each pointing to a page table) */
static page_directory_t page_directory __attribute__((aligned(PAGE_SIZE)));

/* Page tables for kernel space (we pre-allocate first 4 tables for kernel) */
static uint32_t kernel_page_tables[4][1024] __attribute__((aligned(PAGE_SIZE)));

/* Current page directory physical address */
static uint32_t current_page_directory = 0;

/* Paging enabled flag */
static int paging_enabled = 0;

/* Get page directory entry for virtual address */
static inline uint32_t pde_index(uint32_t virtual_addr)
{
    return (virtual_addr >> 22) & 0x3FF;
}

/* Get page table entry for virtual address */
static inline uint32_t pte_index(uint32_t virtual_addr)
{
    return (virtual_addr >> 12) & 0x3FF;
}

/* Flush TLB for a specific virtual address */
static inline void tlb_flush(uint32_t virtual_addr)
{
    __asm__ volatile("invlpg (%0)" : : "r"(virtual_addr) : "memory");
}

/* Flush entire TLB */
static inline void tlb_flush_all(void)
{
    __asm__ volatile(
        "movl %%cr3, %%eax\n"
        "movl %%eax, %%cr3\n"
        : : : "eax", "memory"
    );
}

/* Load page directory into CR3 */
static inline void load_page_directory(uint32_t pd_physical)
{
    __asm__ volatile("movl %0, %%cr3" : : "r"(pd_physical) : "memory");
}

/* Enable paging in CR0 */
static inline void enable_paging_cr0(void)
{
    uint32_t cr0;
    __asm__ volatile("movl %%cr0, %0" : "=r"(cr0));
    cr0 |= 0x80000000;  /* Set PG bit */
    __asm__ volatile("movl %0, %%cr0" : : "r"(cr0) : "memory");
}

/* Get current CR2 (Page Fault Linear Address) */
static inline uint32_t get_cr2(void)
{
    uint32_t cr2;
    __asm__ volatile("movl %%cr2, %0" : "=r"(cr2));
    return cr2;
}

/* Initialize paging */
void paging_init(void)
{
    /* Clear page directory */
    for (int i = 0; i < 1024; i++) {
        page_directory[i] = 0;
    }
    
    /* Identity map first 16MB of memory (kernel space)
     * This maps virtual addresses 0-16MB to physical addresses 0-16MB
     * Uses 4 page tables (each covers 4MB) */
    for (int table = 0; table < 4; table++) {
        /* Clear the page table */
        for (int page = 0; page < 1024; page++) {
            uint32_t physical_addr = (table * 1024 + page) * PAGE_SIZE;
            
            /* Identity mapping with supervisor access */
            kernel_page_tables[table][page] = physical_addr | PTE_PRESENT | PTE_WRITABLE;
        }
        
        /* Add page table to page directory */
        page_directory[table] = (uint32_t)kernel_page_tables[table] | PTE_PRESENT | PTE_WRITABLE;
    }
    
    /* Store the page directory physical address */
    current_page_directory = (uint32_t)page_directory;
    
    print_str("Paging: Page directory at ", WHITE);
    print_hex(current_page_directory, YELLOW);
    print_new_line();
    print_str("Paging: Identity mapped first 16MB", GREEN);
    print_new_line();
}

/* Enable paging */
void paging_enable(void)
{
    if (paging_enabled) {
        return;
    }
    
    /* Load page directory */
    load_page_directory(current_page_directory);
    
    /* Enable paging */
    enable_paging_cr0();
    
    paging_enabled = 1;
    
    print_str("Paging: ENABLED", GREEN);
    print_new_line();
}

/* Get or create page table for virtual address */
static uint32_t* get_page_table(uint32_t virtual_addr, int create)
{
    uint32_t pd_index = pde_index(virtual_addr);
    
    if (page_directory[pd_index] & PTE_PRESENT) {
        /* Page table exists */
        return (uint32_t*)(page_directory[pd_index] & 0xFFFFF000);
    }
    
    if (!create) {
        return NULL;
    }
    
    /* Allocate new page table */
    uint32_t *new_table = (uint32_t*)pmm_alloc_page();
    if (new_table == NULL) {
        kernel_warning("Paging: Failed to allocate page table", __FILE__, __LINE__);
        return NULL;
    }
    
    /* Clear the new page table */
    memset(new_table, 0, PAGE_SIZE);
    
    /* Add to page directory */
    page_directory[pd_index] = (uint32_t)new_table | PTE_PRESENT | PTE_WRITABLE | PTE_USER;
    
    return new_table;
}

/* Map a virtual address to a physical address */
void paging_map_page(uint32_t virtual_addr, uint32_t physical_addr, uint32_t flags)
{
    /* Align addresses to page boundary */
    virtual_addr &= 0xFFFFF000;
    physical_addr &= 0xFFFFF000;
    
    /* Get page table (create if necessary) */
    uint32_t *page_table = get_page_table(virtual_addr, 1);
    if (page_table == NULL) {
        PANIC("Failed to get page table for mapping");
    }
    
    /* Get page table entry index */
    uint32_t pt_index = pte_index(virtual_addr);
    
    /* Check if already mapped */
    if (page_table[pt_index] & PTE_PRESENT) {
        kernel_warning("Paging: Remapping already mapped page", __FILE__, __LINE__);
    }
    
    /* Set the page table entry */
    page_table[pt_index] = physical_addr | flags | PTE_PRESENT;
    
    /* Flush TLB for this address */
    if (paging_enabled) {
        tlb_flush(virtual_addr);
    }
}

/* Unmap a virtual address */
void paging_unmap_page(uint32_t virtual_addr)
{
    /* Align address to page boundary */
    virtual_addr &= 0xFFFFF000;
    
    /* Get page table */
    uint32_t *page_table = get_page_table(virtual_addr, 0);
    if (page_table == NULL) {
        return;  /* Not mapped */
    }
    
    /* Get page table entry index */
    uint32_t pt_index = pte_index(virtual_addr);
    
    /* Clear the page table entry */
    page_table[pt_index] = 0;
    
    /* Flush TLB for this address */
    if (paging_enabled) {
        tlb_flush(virtual_addr);
    }
}

/* Get physical address from virtual address */
uint32_t paging_get_physical(uint32_t virtual_addr)
{
    /* Get page table */
    uint32_t *page_table = get_page_table(virtual_addr, 0);
    if (page_table == NULL) {
        return 0;  /* Not mapped */
    }
    
    /* Get page table entry index */
    uint32_t pt_index = pte_index(virtual_addr);
    
    /* Check if present */
    if (!(page_table[pt_index] & PTE_PRESENT)) {
        return 0;  /* Not present */
    }
    
    /* Return physical address with offset */
    return (page_table[pt_index] & 0xFFFFF000) | (virtual_addr & 0xFFF);
}

/* Allocate a new page table */
page_table_t paging_alloc_table(void)
{
    uint32_t *table = (uint32_t*)pmm_alloc_page();
    if (table) {
        memset(table, 0, PAGE_SIZE);
    }
    return table;
}

/* Print paging information */
void paging_print_info(void)
{
    print_str("=== Paging Information ===", CYAN);
    print_new_line();
    print_str("Page Directory: ", WHITE);
    print_hex(current_page_directory, YELLOW);
    print_new_line();
    print_str("Paging Status:  ", WHITE);
    print_str(paging_enabled ? "ENABLED" : "DISABLED", paging_enabled ? GREEN : RED);
    print_new_line();
    print_new_line();
    
    /* Count mapped page tables */
    int mapped_tables = 0;
    int mapped_pages = 0;
    
    for (int i = 0; i < 1024; i++) {
        if (page_directory[i] & PTE_PRESENT) {
            mapped_tables++;
            uint32_t *pt = (uint32_t*)(page_directory[i] & 0xFFFFF000);
            for (int j = 0; j < 1024; j++) {
                if (pt[j] & PTE_PRESENT) {
                    mapped_pages++;
                }
            }
        }
    }
    
    print_str("Page Tables:    ", WHITE);
    print_dec(mapped_tables, YELLOW);
    print_new_line();
    print_str("Mapped Pages:   ", WHITE);
    print_dec(mapped_pages, YELLOW);
    print_str(" (", LIGHT_GREY);
    print_dec(mapped_pages * 4, WHITE);
    print_str(" KB)", LIGHT_GREY);
    print_new_line();
}
